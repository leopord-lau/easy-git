# easy-git

![git-log](./docs/figures/README/git-logo.png)

Git 是属于「分布式版本控制」系统，本仓库将介绍从如何使用 Git 到基本原理。

## 目录

* [01 - 版本控制背景知识](./docs/01-版本控制背景知识.md)
  
* [02 - git的安装](./docs/02-git%20的安装.md)

* [03 - git的基础指令](./docs/03-%20git%20基础指令.md)

* [04 - git中的概念](./docs//04-git%20中的概念.md)

* [05 - git 中分支的概念及使用](./docs/05-git%20中分支的概念及使用.md)

* [06 - 使用 GitHub 远程仓库](./docs/06-使用%20GitHub%20远程仓库.md)

* [07 - fetch、pull命令详细介绍](./docs/07%20fetch、pull命令详细介绍.md)

* [08 - tag标签](./docs/08-tag标签.md)

* [09 - commit 历史版本记录修正](./docs/09-commit%20历史版本记录修正.md)

* [10 - git 对象压缩及垃圾对象清理](./docs/10-git%20对象压缩及垃圾对象清理.md)

* [11 - .git 文件夹结构解析](./docs/11-.git%20文件夹结构解析.md)

* [12 - 工作中常用的git操作](./docs/12-工作中常用的git操作.md)

* [13 - ssh 配置](./docs/13-ssh配置.md)